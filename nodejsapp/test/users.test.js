...
//user enrolling for a already enrolled course
...
//user droping an unerolled course
test("Droping an unenrolled course",async()=>{
...
test("Rating an already rated course",async()=>{
  await request(app).patch(`/courses/rating/${courseOne._id}`).send({
    rating:5,
  }).expect(403);
});
//user giving rating to a coursehas not  enrolled
test("giving rating to a unerolled course",async()=>{
  await request(app).patch(`/courses/rating/${courseFive._id}`).send({
    rating:5,
  }).expect(403);
});